<?php
	...
	foreach($myposts as $wa_post) {
		...
		$slider_gallery.= '<div class="cartwishcompare"><div class="wa_wps_add_to_cart" id="wa_wps_add_to_cart'.$id.'"><a  rel="nofollow" data-product_id="'.$post_id.'" data-product_sku="'.$_product->get_sku().'" class="wa_wps_button add_to_cart_button product_type_simple" href="'.do_shortcode('[add_to_cart_url id="'.$post_id.'"]').'">+'.__('Add to cart', 'woocommerce').'</a></div>';
		$slider_gallery.= '<div class="joomirwishilisti"><a title="اضافه به لیست علاقه مندیها" >'.do_shortcode('[yith_wcwl_add_to_wishlist]').'</a></div>';
		$slider_gallery.= '<div class="joomircompare"><a title="اضافه به لیست مقایسه" >'.do_shortcode('[yith_compare_button]').'</a></div></div>';
		}
	...
	if($wps_pre_direction=="up"||$wps_pre_direction=="down") {
		...
		$slider_gallery.='<div class="owl-controls clickable"><a class="wa_wps_prev" id="foo'.$id.'_prev" href="#"><span>‹</span></a>';
		$slider_gallery.='<a class="wa_wps_next" id="foo'.$id.'_next" href="#"><span>›</span></a></div>';
